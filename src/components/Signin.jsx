...
const Signin = () => {
    ...
    const initialFormValues = {
        query: "mutation customerAccessTokenCreate($input: CustomerAccessTokenCreateInput!) { customerAccessTokenCreate(input: $input) { customerAccessToken { accessToken, expiresAt} customerUserErrors { field, message, code } } }",
        variables: {
            input: {
                email: "",
                password: ""
    ...
    const signin = async () => {
        await axios.post("https://theodinapitest.myshopify.com/api/2022-01/graphql.json", JSON.stringify(formValues), config)
            .then(res => {
                // console.log("res : ", res);
                if (res.data.data.customerAccessTokenCreate.customerAccessToken !== null) {
                    handleToastMessage("Welcome Back", "green");
                    navigate("/profile");
                    localStorage.setItem("OrchidUser", JSON.stringify(res.data.data.customerAccessTokenCreate.customerAccessToken.accessToken));
                } else {
                    // console.log(res.data.data.customerAccessTokenCreate.customerUserErrors[0].message);
                    handleToastMessage(res.data.data.customerAccessTokenCreate.customerUserErrors[0].message, "red");
                }
            })
            .catch(err => {
                // console.log("err :", err);
        ...
        <>
            <div className="card admin-login-card mb-0 bg-white shadow-0  mx-0 rounded-0 full-height vh-100">
                <div className="card-center">
                    <div className="card card-style overflow-auto" style={{ boxShadow: "none", paddingTop: "120px" }} >
                        <div className="content" style={{ paddingBottom: "60px" }}>
                            ...
                            <Link to="/profile/forgotpassword" className="d-block text-center" >Forgot your password?</Link>
